...
const { newGamePieces, wPawn, bPawn, wKnight, bKnight, wBishop, bBishop, wRook, bRook, wKing, bKing, wQueen, bQueen, } = require("../data/newGamePieces");
...
const { WR1INDEX, WR2INDEX, BR1INDEX, BR2INDEX, WK1INDEX, BK1INDEX, } = require("../data/pieceIndexes");
function moveLogic(selectedPiece, square, pieces, whiteToMove, capturedPieces, moves) {
    ...
    let newWhiteKingSquare = pieces[WK1INDEX].currentCol.toString() +
        pieces[WK1INDEX].currentRow.toString();
    let newBlackKingSquare = pieces[BK1INDEX].currentCol.toString() +
    ...
    if (selectedPiece.type === "king" &&
        ...
        if (selectedPiece.type === "pawn" &&
            ...
            capturePiece(piecesCopy[moves[moves.length - 1].movedPieceIndex], capturedPiecesCopy, piecesCopy);
    ...
    const oppositionKingSquare = selectedPiece.colour === "white" ? newBlackKingSquare : newWhiteKingSquare;
    const calculateAllAvailableMovesResult = calculateAllAvailableMoves(piecesCopy, selectedPiece.colour, oppositionKingSquare, movesCopy);
    ...
    const calculateOppositionLegalMovesResult = calculateOppositionLegalMoves(piecesCopy, oppositionKingSquare, oppositionColour, capturedPiecesCopy, movesCopy, inCheck);
...
function promoteLogic(pieces, selectedPiece, promoteTo, moves, capturedPieces, whiteToMove) {
    let piecesCopy = JSON.parse(JSON.stringify(pieces));
    ...
    let whiteKingSquare = pieces[WK1INDEX].currentCol.toString() +
        pieces[WK1INDEX].currentRow.toString();
    let blackKingSquare = pieces[BK1INDEX].currentCol.toString() +
        pieces[BK1INDEX].currentRow.toString();
    const oppositionKingSquare = selectedPiece.colour === "white" ? blackKingSquare : whiteKingSquare;
    let playerColour = selectedPiece.colour;
    let oppositionColour = playerColour === "white" ? "black" : "white";
    const calculateAllAvailableMovesResult = calculateAllAvailableMoves(piecesCopy, selectedPiece.colour, oppositionKingSquare, moves);
    piecesCopy = calculateAllAvailableMovesResult.pieces;
    const inCheck = calculateAllAvailableMovesResult.inCheck;
    // for each of opposition player's pieces, calculate legal moves.
    const calculateOppositionLegalMovesResult = calculateOppositionLegalMoves(piecesCopy, oppositionKingSquare, oppositionColour, capturedPieces, moves, inCheck);
    const isCheckmate = calculateOppositionLegalMovesResult.isCheckmate;
    const isStalemate = calculateOppositionLegalMovesResult.isStalemate;
    piecesCopy = calculateOppositionLegalMovesResult.pieces;
    if (isCheckmate) {
