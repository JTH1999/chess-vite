...
export function calculateSelectedPieceLegalMoves(
  ...
  for (let i = 0; i < selectedPiece.availableMoves.length; i++) {
    ...
    if (
      ...
      !selectedPiece.moved &&
      square[1] === piecesCopy[selectedPiece.index].currentRow.toString() &&
      ...
      if (piece) {
        ...
        oppositionPiecesCopy[opPieceIndex] = piecesCopy[capturedPieceIndex];
      }
  ...
  for (let j = 0; j < pieces.length; j++) {
    if (
      ...
      if (pieces[j].colour !== piece.colour) {
        const square =
          pieces[j].currentCol.toString() + pieces[j].currentRow.toString();
...
export function checkAvailableMoves(
  ...
  if (piece.type === "pawn") {
    ...
    let twoAhead = "";
    const oneAhead =
      piece.currentCol.toString() + (piece.currentRow + 1 * rowMove).toString();
    ...
    for (let i = 0; i < pieces.length; i++) {
      ...
      const square =
        pieces[i].currentCol.toString() + pieces[i].currentRow.toString();
    ...
    for (let i = 1; i <= rowsLeft; i++) {
      ...
      if (check) {
        ...
        const square =
          (piece.currentCol - i).toString() + piece.currentRow.toString();
        possibleMoves.push(square);
      }
    }
    ...
    for (let i = 1; i <= colsUp; i++) {
      ...
      if (check) {
        ...
        const square =
          piece.currentCol.toString() + (piece.currentRow + i).toString();
        possibleMoves.push(square);
      }
    }
    ...
    let allKnightMoves = [];
    allKnightMoves.push(
      (piece.currentCol + 1).toString() + (piece.currentRow + 2).toString()
    );
    allKnightMoves.push(
      (piece.currentCol + 2).toString() + (piece.currentRow + 1).toString()
    );
    allKnightMoves.push(
      (piece.currentCol + 2).toString() + (piece.currentRow - 1).toString()
    );
    ...
    for (let i = 1; i <= 7; i++) {
      ...
      if (check) {
        ...
        const square =
          (piece.currentCol - i).toString() + (piece.currentRow + i).toString();
        possibleMoves.push(square);
      }
    }
    ...
    allKingMoves.push(
      (piece.currentCol + 1).toString() + (piece.currentRow + 1).toString()
    );
    allKingMoves.push(
